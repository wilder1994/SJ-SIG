...
| **Clientes / Usuarios** | Tabla de clientes (ciudad, estructura, sedes, personal, usuarios) + Entrar al tablero + ficha/mapa. Usuarios del universo | Solo administración |
...
Flujo actual: Clientes (tabla → **Entrar** o Editar) → Usuarios / Instalaciones → Tablero (mapa satélite, navy = cliente, cian = sedes) → Personal (tabla o **Carga masiva**) → ficha / Editar → Documentos → Ver carpeta (tarjetas) → **Cargar documentos**.
