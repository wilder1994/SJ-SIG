...
<section class="upload-deck" style="margin-bottom:12px">
    ...
    <p class="muted" style="margin-bottom:12px">Arrastre, pegue o seleccione. Historia Laboral y Afiliaciones pasan al indexador. El escáner queda pendiente.</p>
    <div class="drop-grid">
        ...
        <form class="drop-card" method="post" action="{{ route('documents.affiliations.batch', $person) }}" enctype="multipart/form-data" data-dropzone data-accept=".pdf,application/pdf" data-max="20480">
            @csrf
            <p class="drop-card-title">Afiliaciones</p>
            <p class="muted drop-card-hint">{{ \App\Enums\DocumentFolder::Afiliaciones->hint() }}</p>
            <input class="drop-input" type="file" name="file" accept=".pdf" required>
            <div class="drop-empty">
                <span class="drop-icon drop-icon-pdf" aria-hidden="true">PDF</span>
                <p>Arrastre, pegue o seleccione un PDF</p>
...
<section class="split">
    ...
    <article class="card">
        ...
        <div class="history-head" style="margin-top:16px">
            <div>
                <p style="margin:0;font-weight:500">Afiliaciones</p>
                <p class="muted">{{ $affiliationSummary['loaded'] }}/{{ $affiliationSummary['total'] }} indexados · {{ $affiliationSummary['required_loaded'] }}/{{ $affiliationSummary['required'] }} obligatorios</p>
            </div>
            <button class="btn ghost" type="button" data-toggle-panel="affiliation-list">Listado</button>
        </div>
        <div id="affiliation-list" hidden>
            <table class="data" style="margin-top:10px">
                <thead><tr><th>Tipo</th><th>Estado</th><th></th></tr></thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="white-space:nowrap">
                            @if($row['status'] === 'loaded' && $row['document'])
                                ...
                                <a class="btn ghost" href="{{ route('documents.download', $row['document']) }}">Descargar</a>
                            @elseif($cargar && $row['status'] !== 'na')
                                <form method="post" action="{{ route('documents.affiliations.na', $person) }}" style="display:inline">
                                    @csrf
                                    <input type="hidden" name="document_type" value="{{ $row['type']->value }}">
                                    <button class="btn ghost" type="submit">No aplica</button>
                                </form>
